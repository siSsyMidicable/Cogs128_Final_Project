...
import { getState, getSwapHistory, type SwapRecord } from '@/lib/matching/matching';
...
export default function OutgoingScreen() {
  const completed = MOCK_USERS.filter(u => getState(u.id) === 'completed');
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {completed.length === 0 ? (
          ...
          const history: SwapRecord[] = getSwapHistory(user.id);
          const last = history[history.length - 1];
          const ratingGiven = last?.ratingGiven ?? 0;
          const fairness = last?.outcome
            ? (last.outcome.deliveredOnTime ? 1 : 0) +
              (last.outcome.scopeMatchedAgreement ? 1 : 0) +
              (last.outcome.wouldSwapAgain ? 1 : 0)
            : 0;
            ...
            <Island key={user.id} accent={fairColor}>
              ...
              {last && (
                <View style={s.skillRow}>
                  <Text style={s.skillChip}>🤝 {last.youGave} ↔ {last.theyGave}</Text>
                </View>
              )}
              ...
              {last?.notes && (
                <View style={s.notesBox}>
                  ...
                  <Text style={s.notesText}>"{last.notes}"</Text>
                </View>
              )}
